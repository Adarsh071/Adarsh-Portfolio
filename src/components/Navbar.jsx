...
const Navbar = ({darkMode, toggleDarkMode}) => {
  ...
  const lightColors = {
    ...
    indiator: 'from-orange-500 to-amber-500',
              ...
              {navItems.map((item) => (
                ...
                >
                  ...
                  {activeSection === item.name.toLowerCase() && (
                    ...
                    className={`absolute -bottom-1 left-0 right-0 h-0.5 bg-linear-to-r rounded-full ${colors.indiator}`}>
                    </motion.div>
                  )}
                </a>
              ))}
            ...
            ${darkMode
              ...
              <div className="px-4 py-3 space-y-2">
                {navItems.map((item) => (
                  ...
                  key={item.name}
                  href={item.link}
                  onClick={() => handleNavClick(itemName)}
                  className="block">
                    <motion.div 
                    whileHover={{x: 5}}
                ...
                whileTap={{scale: 0.95}}
                className={`block py-3 px-4 text-center font-semibold rotate-lg bg-linear-to-r ${colors.button} text-white shadow-md`}>
                  Hire Me
                </motion.a>
              </div>
            </motion.div>
